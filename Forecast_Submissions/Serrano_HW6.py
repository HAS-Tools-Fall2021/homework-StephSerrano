...
#2. Time series of flow values with the x axis range limited
#Time series of flow values from May 15, 1990 to Dec. 10, 2012
fig, ax = plt.subplots()
ax.plot(data['datetime'], data['flow'], label='flow')
ax.set(title="Observed Flow 1990-2012", xlabel="Date", ylabel="Weekly Avg Flow [cfs]",
        yscale='log', xlim=[datetime.date(1990, 1, 26), datetime.date(2012, 12, 10)])
ax.legend()
plt.show()

#Time series of flow values from Jan. 1, 1989 to Oct. 2, 2021
fig, ax = plt.subplots()
ax.plot(data['datetime'], data['flow'], label='flow')
ax.set(title="Observed Flow Complete", xlabel="Date", ylabel="Weekly Avg Flow [cfs]",
        yscale='log', xlim=[datetime.date(1989, 1, 1), datetime.date(2021, 10, 2)])
ax.legend()
plt.show()
...
#linewidth really just changes how thick you want the lines to look
#second half changes the axis titles

# 4. September flows for 1990 to 2000
#different colour palettes: plasma, inferno, magma, cividis, spring, summer, autumn, winter, binary, etc.
...
#5. Scatter plot of 2000 flow vs 2001 flow for the month of December
fig, ax = plt.subplots()

ax.scatter(data[(data['year'] == 2000) & (data['month'] == 12)].flow,  data[(data['year'] == 2001) & (data['month'] == 12)].flow, marker='p',
           color='blueviolet')
ax.set(xlabel='2019 flow', ylabel='2020 flow', yscale='log', xscale='log')
ax.legend()

# 6. Scatter plot of flow vs day for December using the magma colour palette
# Dots are colored by the year and sized acccording to the flow
dec_data = data[data['month']==12] 
fig, ax = plt.subplots()
ax.scatter(dec_data['day'], dec_data['flow'], alpha=0.2,
            s=0.02*dec_data['flow'], c=dec_data['year'], cmap='magma')
ax.set(yscale='log')
ax.set_xlabel('Day of the month')
ax.set_ylabel('Flow')
plt.show()

#7. Multipanel plot histograms of flow for November and December
fig, ax = plt.subplots(1,2)

m = 11
month_data = data[data['month'] == m]
plot_title = 'November'
ax[0].hist(np.log10(month_data['flow']), bins=30, edgecolor='grey', color='purple')
ax[0].set(xlabel='Log Flow (cfs)', ylabel='Count', title=plot_title)
...
#8. Same as 7 but using a for loop to do all 12 months
#for loop that shows all 12 months
