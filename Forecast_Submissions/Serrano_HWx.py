...
#____________________ Forecast #4 Code ________________
#Repasting this 
# Flow data has a row for every day and 4 columns:
# 1. Year
# 2. Month
...
# Question 2
flow_count = np.sum((flow_data[:,3] > 95) & (flow_data[:,1]==9))
print(flow_count)

# same thing but split apart into sep. lines
flow_test = flow_data[:, 3] > 95  # Note that this returns a 1-d array that has an entry for every day of the row with either a t/f
month_test = flow_data[:, 1] ==9   # same as above but for september [9]
combined_test = flow_test & month_test  # now looking at our last two tests and finding when they are both true
flow_count = np.sum(combined_test) # add up all the array (note Trues = 1 and False =0) so by default this counts all the times our criteria are true
print(flow_count)

flow_test = flow_data[:, 3] >95  # Note that this returns a 1-d array that has an entry for every day of the timeseies (i.e. row) with either a true or a fals
year_test = flow_data[:, 1] ==2000   # doing the same thing but testing if month =7 
combined_test = flow_test & year_test  # now looking at our last two tests and finding when they are both true
flow_count = np.sum(combined_test) # add up all the array (note Trues = 1 and False =0) so by default this counts all the times our criteria are true
print(flow_count)

flow_test = flow_data[:, 3] >95
year_test = flow_data[:, 1] ==2010
combined_test = flow_test & year_test
flow_count = np.sum(combined_test)
print(flow_count)
...
# average summary stuff
flow_mean = np.mean(flow_data[(flow_data[:,3] > 95) & (flow_data[:,1]==9),3])

# same thing but split apart into sep. lines
criteria = (flow_data[:, 3] > 95) & (flow_data[:, 1] == 9)  # Shows an array of t/f values for met daily entries
flow_pick = flow_data[criteria, 3] #Grab out the flow/4th column for every row where the criteria was true
flow_mean =  np.mean(flow_pick) # takes the average of the values from before
...
#bins
mybins = np.linspace(0, 1000, num=18)
...
#lil test
mybins = np.linspace(0, 1000, num=365)
